...
class ComplianceQuota(models.Model):
    ...
    def accomplished(self):
        """Calculate accomplished inspections from finished inspections in this quarter"""
        from datetime import datetime
        
        # Get quarter date range
        quarter_start, quarter_end = self.get_quarter_dates()
        
        # Count finished inspections for this law in this quarter
        finished_statuses = [
...
class ComplianceQuota(models.Model):
    ...
    def get_quarter_dates(self):
        ...
        if self.quarter == 1:
            ...
            end = datetime(self.year, 3, 31, 23, 59, 59)

        elif self.quarter == 2:

            start = datetime(self.year, 4, 1)

            end = datetime(self.year, 6, 30, 23, 59, 59)

        elif self.quarter == 3:

            start = datetime(self.year, 7, 1)

            end = datetime(self.year, 9, 30, 23, 59, 59)

        else:  # quarter == 4

            start = datetime(self.year, 10, 1)
